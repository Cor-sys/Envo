      ...
      <section className="space-y-2 report-section">
        ...
        <div className="surface print:border-slate-300 divide-y divide-slate-800 print:divide-slate-300">
          {summary.byType.map((row) => (
            <div key={row.type} className="flex items-center justify-between px-3 py-2 text-sm">
              <div className="text-slate-200 print:text-slate-900">{itemTypeLabel(row.type)}</div>
            ...
            <tbody className="divide-y divide-slate-800 print:divide-slate-300">
              {reorder.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="py-1 pr-2 text-slate-400 print:text-slate-700">{itemTypeLabel(r.item_type)}</td>
                ...
                <tbody className="divide-y divide-slate-800 print:divide-slate-300">
                  {groupItems.map((it) => (
                    <tr key={it.id}>
                      ...
                      <td className="py-1 pr-2 text-slate-400 print:text-slate-700 text-xs">
                        {it.location_text ?? ''}
                      </td>
